...
- **Renew and inspect.** `lo registry tls renew` mints a new certificate into the volume and restarts the set's containers. Run it after a `CAROOT` change, or when you want a fresh leaf. `lo registry tls status` prints the SANs, the validity dates and what each container mounts. See [`lo registry tls`](/reference/cli#lo-registry-tls).
...
- **Upgrading from a release before v0.4.0.** The certificate lived in `.secrets/tls/registries/`. The first `lo up` imports those files into the volume and warns that the running containers still mount the directory. Run `lo registry down && lo registry up` to switch the mounts. Then remove `.secrets/`. `lo doctor` reports which mount the containers use.
