...
| `connectHcloudToken` | `false` | When `true` **and** a `KUBEHZ_TOKEN` is set, `lo provision` also stores your `HCLOUD_TOKEN` with the platform (encrypted at rest, used only to manage your clusters) so the dashboard can drive provisioning (worker pools, SSH keys). Without it `lo` uses the token only locally and never sends it. A read-only token gets stored for account-exact pricing but leaves provisioning locked. |
...
the api's `503 AT_CAPACITY`. The message names what is full and counts it
the way that gate counts: slots in use and free for a shape or for the free
pool, and nodes present against nodes required when a cluster asks for
dedicated hardware. Then it gives the ways out: retry later (with a
suggested wait), pick a smaller shape with fewer apiserver replicas or drop
the option, watch the live capacity, run a self-hosted cluster meanwhile, or
contact support.
...
- **`channel`** is consent, not a schedule: `patch` (the default) allows
  ...
  end-of-support floor. The platform still upgrades clusters that run
  releases too far behind, regardless of channel.
...
- **Nodes you own** (self-hosted clusters, and machines you registered
  yourself) get upgraded **in place**: the upgrade swaps the kubelet
  underneath the running workloads, and the swap itself does not restart
  them. On the managed tier, the live agent will walk the nodes one at a
  time once upgrade execution ships.
- **Hosted worker pools** get **replaced, surge-first**: new nodes join on
...
block is declarative only. `lo` does not catch calendar validity beyond
the shape (a February 30th) today.
...
- **The shard is the upgrade unit.** The platform upgrades the shared
  control plane as a whole, on its own schedule, and retires old versions on
  that schedule too. Here `upgrades.channel` is a preference, not consent: an
  eager channel moves your space early in a shard's upgrade window, a
  conservative one later, but the platform migrates every space by the
  deadline. There is no opting out.
...
The platform docs cover joining a node (requirements, the recommended host
firewall, the bare-metal lane):
...
lok8s applies the same bootstrap DAG a self-hosted cluster gets: your
...
- **You need working kubectl access**: the default hosted kubeconfig is
  ...
  clear notice and a skipped bootstrap. Install the kubelogin
...
- **Workers first**: a control plane with no Ready workers can't run addon
  workloads, and `wait:`-gated entries would hang, so `lo` skips the
...
| `--node-ip` | (none) | The address other nodes reach this machine on. Add it when the machine's default-route address is not the one the cluster can dial. |
...
a lost ticket gets minted again, never recovered. A second mint for the same
...
This does not list nodes in kubehz-provisioned pools: those appear as pool
counts on the cluster itself. A warning appears while the control plane has
published no join address: the platform refuses a mint until it does.
...
Removal takes the **membership**, never the hardware. The removal does not
evict pods on the machine first, because the machine is yours and asking for it back is not
...
A node that stops reporting keeps its slot until you remove it. The platform
...
agent generates the secrets **once** and never rotates them, so re-applying it
is a no-op. Registration is **provider-agnostic**: no SSH key, no hcloud token,
...
are the real path) and stays only as a best-effort announce and as the
...
**provider-agnostic and needs no hcloud token**: you prove ownership when you
read a one-time code out of the cluster (which only its operator can do) and
paste it into the dashboard while signed in.
...
3. **Paste it** into the dashboard **Claim** page while signed in. The platform
   attributes the cluster to your account and consumes the code (single-use).
...
leaves the cluster, and no command prints it, `lo kubehz claim-code`
included.
...
- **Worker scaling, self-healing and worker upgrades**, when **all** of these
  ...
  acting on (there is no flag for it), and without `access: managed` the
  platform grants no acting permission at all.
...
- `patch` on `machinedeployments.cluster.k8s.io`, two fields only: the replica
  count (scaling) and the kubelet version (worker upgrades). No `create`, no
  `delete`.
...
CronJob's bootstrap once as a one-off Job and waits for it, so the live agent
finds its identity Secret at once instead of at the CronJob's next tick.
...
- **Self-bootstrapping identity.** On each run the agent ensures one Secret,
  ...
  (`khz_agt_…`) and claim-code `C` (`khzc_…`). The agent creates it once and
  **never rotates** it; the platform only ever receives `sha256(A)`/`sha256(C)`.
...
- **Least-privilege RBAC.** The `kubehz-agent` ServiceAccount can read cluster
  ...
  + `/version` API paths. On top of that it holds `get`+`create` (never
  `update`/`patch`/`delete`) on Secrets in its own `kubehz-system` namespace,
  to bootstrap its identity Secret once. It reads no Secrets elsewhere and can rotate nothing. The
...
(**Access → API Tokens**). When you set it during `lo provision`, the platform
**claims the cluster directly to your tenant at registration**: no claim code
to paste, no fingerprint step. It shows up in your dashboard already owned. Pair
...
of its RBAC there. The cluster-scoped roles go in the second command. Both
