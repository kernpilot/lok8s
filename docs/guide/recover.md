...
Kubernetes cluster: it reimages the nodes in place (IPs, network, and
load-balancer preserved), then runs a fresh `lo provision`. It is the
foundation that a disaster recovery stands on: it restores the **cluster**, not the
application **data** (etcd snapshots, databases, PVs restore *on top* of a
recovered cluster; see [Backups](./backups.md)).
...
4. **rebuild**: [`provider::rebuild`](./bare-metal.md#diagnose-infrastructure-lo-doctor)
   resets the cluster's existing nodes from bare metal (cloud VMs reimaged via
...
5. **provision**: a fresh `lo provision` (KubeOne/CAPI install). The bare-metal
   path runs the `#wipe-devices` data-disk wipe transparently.
...
- **`provider::rebuild` enforces.** Its atomic preflight is the hard safety gate:
  ...
  wrong machines, or only some of them).
...
It skips the readiness barrier because no node changed state. It then reports
