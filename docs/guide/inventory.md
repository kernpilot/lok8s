...
inventory of their own; the referenced cluster's inventory gets written when
that cluster provisions.

Both the CRD and the CR apply idempotently (server-side apply), so
...
`spec.addons[].chartVersion` against the index) without access to the
