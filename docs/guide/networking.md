...
e.g. `gateway.yaml`), and `lo bootstrap` / `lo provision` applies them.
...
Hetzner cluster. On a cluster that brings its own storage (Ceph via Rook, or any
...
- **Clusters that relied on the hcloud CSI** (PVCs on `hcloud-volumes`): set
  ...
  otherwise those PVCs become unmountable once the driver is gone.
:::
...
shadows the embedded `csi-hetzner` and KubeOne skips an empty addon dir as a
no-op, so the embedded CSI never applies. (The override must be an empty _directory_:
