...
additions to existing commands: `lo init project` (with `--cluster` and
`--driver`), the `lo init` wizard on a terminal and `lo init --plan`,
`lo lint --notes`, `lo addons --origin`, `lo drivers --list --origin`, the
...
| D32 | **Bare `lo init` on a terminal runs a wizard.** With stdin and stdout on a TTY, `CI` unset and no `--yes`, the binary reads the room (`internal/initctx`: the project root by marker, a subdirectory, a service directory, a submodule under the umbrella, git, what exists) and asks what the situation calls for with charm.land/huh/v2; nothing is written before the summary is confirmed, and every answer is executed through the subcommands' own functions (`lo init project --env/--cluster/--driver`, `lo init service`, `lo init test`, `lo toolchain install --groups`, `lo assets eject bash`, `lo use`), which the summary prints. Off a terminal, under `CI` or with `--yes`, bare `lo init` prints the help and exits 0 as the bash tree does (its help text gains the three flags). `lo init --plan` prints the state card and the commands the defaults would run, rc 0, no writes, on and off a terminal; `--dry-run` off the wizard (no terminal, `CI`, `--yes`) does the same, there is no conversation to stop. The implementation switch has a `lo` twin, `lo init project --implementation go|bash`. No environment variable selects any of it. Go-only. | `internal/initctx`, `internal/cli/cmd_init_wizard.go`, `internal/scaffold/cluster.go`, `hack/parity-configure.sh` (the `--plan` contract cases) |
...
| D34 | **The registry TLS certificate lives in the docker volume `<network>-registry-tls`, not in a project directory.** The bash mints it through the Secret plugin with `PATH_SECRETS` at the flat store its entrypoint defaults (`<Base>/.secrets`), extracts the pair to `<Base>/.secrets/tls/registries/` and bind-mounts that directory into every registry container; with `PATH_SECRETS` unset (a PATH-only shell) the binary's port of that mint failed with `PATH_SECRETS is not set` (v0.3.1). The binary hands the generator a scratch `PATH_SECRETS` under the domain dir (`clusters/<domain>/.registry-tls-tmp.*`, removed on exit), copies `tls.crt`, `tls.key` and `.sans` into the volume through a throwaway container (`docker container create` + `docker cp` + `docker rm`), and every container of the set mounts the volume at `/etc/registry/certs`. Every `lo up` reads `tls.crt` and `.sans` back the same way (the config hash, the re-mint key); the key never leaves the volume, nothing in the cluster consumes it (the kind nodes trust the dev CA through certs.d). A missing volume next to a legacy `<Base>/.secrets/tls/registries/tls.crt` is imported byte for byte with one `[warn]`; the files stay until the operator runs `lo registry down && lo registry up`. `lo registry tls status` and `lo registry tls renew` are Go-only; `lo doctor` gains a line naming the mount the set's containers use. `lo lint`'s flat-shadow line drops its `(it keeps only global TLS like registries-tls)` clause. Parity: `hack/parity-orchestrate.sh` runs one TLS `lo up` as a deviation case (the registry `docker run` argv differs by the `--volume` source, pinned on the Go side), `hack/parity-configure.sh` excludes the shadow line from its diff and pins the Go text. | `internal/driver/lo/registrytls.go`, `internal/driver/lo/registries.go`, `internal/cli/cmd_registry.go`, `internal/cli/cmd_doctor.go`, `internal/secrets/secrets.go` |
