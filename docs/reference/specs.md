...
| `spec.kubehz.maintenanceWindow` | no | — | When platform-driven work may run: `enabled`, `daysOfWeek`, `startTime` (`HH:MM`), `durationMinutes`, `timezone` (IANA name), `exclusions` (absolute freezes — a `YYYY-MM-DD` date or `from/to` range). Declarative passthrough: the platform validates the block at registration; `lo` shape-checks only the two `upgrades` enums and the exclusion dates |
