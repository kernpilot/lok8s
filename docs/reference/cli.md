...
**Without a project here** (an empty directory, a directory without a project, a place below a git root without a project), or **in a project without a git repository**, `lo init` prints one welcome line and then the bootstrap screen. Every value on the screen has a default: the project name (the directory's name, or the project's), the first cluster domain (`<name>.dev`; none when the project has clusters), the driver (`lo`), the environment file (`mise.toml`, or the one that exists), the toolchain (an install now, with the count of pinned tools; in a project only when a pin is missing) and `git init` (when git exists and there is no repository). In a project the scaffold keeps every existing file, and the screen lists only the files the project lacks. Under the values, the screen lists the files it writes (`writes`), the commands it runs (`runs`) and, dim, the equivalent commands (`equivalent`). Then it asks one thing: `Create`, `Change details` or `Cancel`. `Change details` turns the values into fields, one per row. A help line under each field says what the value is and gives an example. Then the screen appears again. `Create` runs the plan and continues in project mode, with the card refreshed and a result line under it. `lo init` writes nothing before `Create`. `Cancel` and `Esc` write nothing and exit 1; `Ctrl-C` writes nothing and exits 130. A failed step does not end the run: project mode shows `failed` and `not run` rows under the card.

**In a project**, `lo init` runs project mode: the card, then one choice from what the project state allows, until `Exit`. The card has the layout of the `lo up` header: one row per fact, the key on the left, the values joined with `·`. The first row is the project: its name, the implementation, the branch and the count of uncommitted paths. The next rows are the clusters (grouped by driver, the active domain first), the toolchain (`.bin/b.yaml` and the count of pinned tools, or the count of missing ones), the environment file (and the bash tree, when present), `services.yaml` and `tests/`. A row that starts with `!` needs an action; the command is in the list's `equivalent` row and in `--plan`. The card prints a path in two cases only: the working directory is not the project root (`directory`, relative to the root), or the repository root is not the project root (`repository`). On a terminal the keys are dim and the `!` is yellow. Off a terminal the card is plain text.

The list offers `Add a cluster`, `Add a service`, `Add the test suite` (when there is no `tests/`), `Install the toolchain` (when `.bin/b.yaml` is missing or unreadable, or a pinned tool is missing, with the count of missing tools), `Set the active domain` (with several clusters, or when `clusters/.active` is absent or names a cluster without a spec), `Eject the bash tree` (when `.lok8s/lo` is absent) or `Switch implementation` (when it is present), and `Exit`. A new cluster becomes the active domain when the project has no valid one. An entry opens its screen: the values (prefilled, or a question when the screen needs one), `writes`, `runs`, `equivalent`, then `Create`, `Change details`, `Cancel`. When the action is done, `lo init` reads the state again and prints the card again, with a one-line result under it (`added  clusters/staging.dev/cluster.lok8s.yaml · active`). `Cancel` returns to the list. `Exit` prints the `next` line and exits 0. The `next` line names the step the state calls for: `lo toolchain install`, `lo init cluster`, `lo use <domain>`, `lo assets diff`, `lo up`, or `lo status`. `Esc` on the list exits 0 too; `Ctrl-C` exits 130. An action writes only on its `Create`, so an exit never leaves a half-written state. Two `lo init` runs in the same project are not locked against each other: the last write to `clusters/.active` wins.
...
`--format text|editor|github` (Go-only) selects the shape of a finding line. `text` prints as before. `editor` prints `<file>:<line>: [error] <message>`, the shape Vim, Emacs and the IDEs jump to. `github` prints `::error file=<file>,line=<line>::<message>`, the workflow command GitHub Actions turns into an annotation. The file is the path the finding names, else the spec file of the domain. The line is the one the message names, else `1` (the checks work per file). Under `GITHUB_ACTIONS=true` with the process stdout off a terminal (a workflow step) the default is `github`. The environment selects a shape of the same findings, never a code path, and `--format text` overrides it. The auto-select reads the real stdout only: a caller that supplies its own writer (the MCP server, a test) gets `text`. The exit code is the same in every format.
