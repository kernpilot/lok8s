...
# to `curl … | sh` from https://lok8s.io/lo-up. `.archive/legacy/install/build` regenerates it
# from `.archive/legacy/install/lo-up`. Nothing in CI notices when the two disagree (issue #112,
# part 2): ci.yml's `e2e-lo-up` job runs `lo up`, which is a different thing
# entirely. So an edit to .archive/legacy/install/lo-up that is never rebuilt keeps shipping the
...
# arg-sh/argsh out at .archive/legacy/install/argsh.pin, downloads the pinned `minifier`
# release asset, runs .archive/legacy/install/build and diffs. What blocked it was not the
...
# that has nothing to do with lo-up. .archive/legacy/install/argsh.pin records that revision.
#
...
setup() {
  ...
  SOURCE="${_PROJECT_ROOT}/.archive/legacy/install/lo-up"
}
...
@test "the published bundle's required-binary list matches the source exactly" {
  ...
  [ "${want}" = "${have}" ] || {
    ...
    echo "  .archive/legacy/install/lo-up : ${want}" >&2
    echo "  docs/public/lo-up  : ${have}" >&2
    echo "Every 'curl | sh' user is being served the old script." >&2
    echo "Rebuild it:  ARGSH_SRC=/path/to/arg-sh/argsh .archive/legacy/install/build" >&2
    return 1
  }
}
...
_pin_value() {
  sed -n "s/^${1}=//p" "${_PROJECT_ROOT}/.archive/legacy/install/argsh.pin"
}

@test ".archive/legacy/install/argsh.pin records a runtime revision" {
  ...
  [[ "${ref}" =~ ^[0-9a-f]{40}$ ]] || {
    echo ".archive/legacy/install/argsh.pin has no 40-char ref (got '${ref}')" >&2
    return 1
  }
  [ -n "${describe}" ] || { echo ".archive/legacy/install/argsh.pin has no describe" >&2; return 1; }
}

@test "the published bundle carries the PINNED argsh runtime" {
  # The bundle embeds `ARGSH_VERSION="…"` from the argsh checkout .archive/legacy/install/build
  ...
  [ -n "${have}" ] || {
    ...
    echo ".archive/legacy/install/lo-up.min.tmpl, or the template no longer stamps it." >&2
    return 1
  }
  [ "${want}" = "${have}" ] || {
    echo "the published installer was built from a DIFFERENT argsh runtime:" >&2
    echo "  .archive/legacy/install/argsh.pin : ${want}" >&2
    echo "  docs/public/lo-up  : ${have}" >&2
    echo "Rebuild:  ARGSH_SRC=/path/to/arg-sh/argsh .archive/legacy/install/build" >&2
    return 1
  }
}
...
@test ".archive/legacy/install/build refuses to build from an unpinned argsh revision" {
  ...
  local build="${_PROJECT_ROOT}/.archive/legacy/install/build"
  grep -q 'argsh.pin' "${build}" || {
    echo ".archive/legacy/install/build no longer reads argsh.pin — the pin is now" >&2
    echo "documentation, not a constraint." >&2
    return 1
  }
  grep -q 'argsh revision does not match .archive/legacy/install/argsh.pin' "${build}" || {
    echo ".archive/legacy/install/build no longer FAILS on a pin mismatch." >&2
    return 1
  }
}
...
@test "CI rebuilds the bundle and diffs it" {
  ...
  grep -q '.archive/legacy/install/build' "${ci}" || {
    echo "no CI step runs .archive/legacy/install/build — docs/public/lo-up can drift from" >&2
    echo ".archive/legacy/install/lo-up with nothing to notice (issue #112 part 2)." >&2
    return 1
  }
