#!/usr/bin/env bats
# loup_verify_test.bats — unit tests for .archive/legacy/install/lo-up's post-step verification
#
...
setup() {
  ...
  eval "$(sed -n '/^loup::bin_dir()/,/^}/p;/^loup::verify_registered()/,/^}/p;/^loup::verify_materialised()/,/^}/p' \
    "${_PROJECT_ROOT}/.archive/legacy/install/lo-up")"
...
@test ".archive/legacy/install/lo-up has ONE basename-of-PATH_BIN expression" {
  ...
  local hits
  hits=$(grep -n '\${PATH_BIN##\*/}' "${_PROJECT_ROOT}/.archive/legacy/install/lo-up" \
    | grep -v '^[0-9]*:#' || true)
  [ -z "${hits}" ] || {
  ...
  grep -q '^loup::bin_dir()' "${_PROJECT_ROOT}/.archive/legacy/install/lo-up" || {
...
@test "lo-up calls verify_registered outside the bootstrap branch" {
  local src="${_PROJECT_ROOT}/.archive/legacy/install/lo-up"
...
@test "the published bundle is in sync with .archive/legacy/install/lo-up" {
  ...
  grep -q 'registered no lok8s env' "${bundle}" \
    || fail "docs/public/lo-up predates the verification guards — re-run .archive/legacy/install/build"
