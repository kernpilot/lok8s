...
lo::write_certs_d() {
  ...
  # `lo up`/provision from cluster node IPs + the local mkcert CA — pure derived
  # state that must never be committed. Drop a gitignore so consumer projects
  # don't need a hand-maintained root rule (mirrors secrets::_ensure_gitignore;
  # but ignores EVERYTHING — unlike the secrets store, nothing here is tracked).
  local containerd_dir="${certs_d%/certs.d}"
  if [[ ! -f "${containerd_dir}/.gitignore" ]]; then
    cat > "${containerd_dir}/.gitignore" <<'GI'
# Auto-generated by lok8s — containerd registry config, regenerated every
# `lo up`/provision from cluster node IPs + the local CA. Never commit it.
*
GI
  fi
