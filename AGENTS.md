...
| init wizard | `internal/initctx`: `Detect` (where a bare `lo init` stands), `Decide` (State + Answers to the ordered actions, each with its flag-twin command), `Ask` and `Confirm` (the charm.land/huh/v2 forms, scripted in tests through huh's accessible mode), the card and the summary. `internal/cli/cmd_init_wizard.go`: the wiring, `--plan`, `--dry-run`, `--yes`, the executor over the subcommands' own functions | none. The frozen tree prints the usage, and so does the binary off a terminal |
