...
| scaffolding | `internal/scaffold` (+ `templates/`, `project.go` for `lo init project`: files only, one env file from one `EnvSpec`; `cluster.go`: the first cluster spec behind `--cluster/--driver`), `internal/crds`, `internal/addons` | `.lok8s/libs/{init,crds,addons}` |
| init screens | `internal/initctx`: `Detect` (where a bare `lo init` stands), `Decide` and the verb plans (State + answers to the ordered actions, each with its flag-twin command), the card (`card.go`), the screens (`screen.go`: the bootstrap, a cluster, a service, the test suite, the toolchain, the active domain, the bash tree, the implementation), project mode (`loop.go`), the charm.land/huh/v2 forms under bubbletea (`form.go`; scripted in tests through huh's accessible mode). `internal/cli/cmd_init_wizard.go`: the wiring, `--plan`, `--dry-run`, `--yes`, the executor over the verbs' own functions; `cmd_init.go`: the verbs (`cluster` is Go-only) | none. The frozen tree prints the usage, and so does the binary off a terminal |
