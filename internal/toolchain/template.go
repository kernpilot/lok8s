...
// template.go — the consumer .bin/b.yaml `lo toolchain install` writes. The
...
// Marker is the header line that identifies a b.yaml written by `lo
// toolchain install`. `lo doctor` verifies the pinned toolchain by default
// only when it is present (a hand-written or profile-synced b.yaml is not
// checked unless --toolchain asks), and the file is never overwritten.
const Marker = "# lo-toolchain: managed pins — written by `lo toolchain install`"

// legacyMarker is the line `lo init toolchain` wrote before WP9 moved the
// command. A file that carries it is still ours: HasMarker accepts both,
// so an existing project keeps its doctor section.
const legacyMarker = "# lo-toolchain: managed pins — written by `lo init toolchain`"
...
// DefaultGroups is the `lo toolchain install` default selection.
...
func Template(o TemplateOptions) (string, error) {
	...
	fmt.Fprintf(&b, "# Generated by lo %s (%s); `lo toolchain install` never overwrites this file, it\n", vPrefixed(o.LoVersion), o.Variant)
	...
	b.WriteString("# and `.bin/b install` to opt in (or: lo toolchain install --groups core,local,cloud,bash).\n")
...
func HasMarker(path string) bool {
	...
	for line := range strings.SplitSeq(string(raw), "\n") {
		if l := strings.TrimSpace(line); l == Marker || l == legacyMarker {
			return true
		}
	}
	return false
...
func Write(bin string, content string, dryRun bool) (WriteResult, error) {
	...
	if raw, err := os.ReadFile(path); err == nil {
		...
		res.Diff = unifiedDiff("b.yaml (existing)", "b.yaml (lo toolchain install)", string(raw), content)
		return res, nil
	}
	if dryRun {
		return res, nil
