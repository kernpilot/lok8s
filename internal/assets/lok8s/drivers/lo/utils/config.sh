...
lo::read_remote_config() {
  ...
  # break out of the quoting on the remote host. Plain path charset only; no
  # ~ either, since single quotes suppress tilde expansion on the remote (a
  # '~/x' dest would be taken literally and fail at runtime).
  if [[ ! "${LOK8S_REMOTE_SYNC_DEST}" =~ ^[A-Za-z0-9_./+-]+$ ]]; then
    error "spec.remote.sync.dest must be a plain absolute/relative path ([A-Za-z0-9_./+-], no ~), got: ${LOK8S_REMOTE_SYNC_DEST}"
    return 1
  fi
