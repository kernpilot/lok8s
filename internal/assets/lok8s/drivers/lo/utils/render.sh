...
lo::write_certs_d() {
  ...
  # `lo up`/provision from cluster node IPs + the local mkcert CA — derived state
  # whose ENTRIES must never be committed. Drop a gitignore so consumer projects
  # don't need a hand-maintained root rule (mirrors secrets::_ensure_gitignore:
  # ignore everything, but keep `!.gitignore` so the file is committable and the
  ...
  local gitignore="${containerd_dir}/.gitignore"
  if [[ ! -f "${gitignore}" ]] || ! grep -qFx '!.gitignore' "${gitignore}"; then
    cat > "${gitignore}" <<'GI'
