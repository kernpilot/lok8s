...
| 2 | `error 20: unable to get local issuer certificate` (`*.<domain>` or registry push) | the shared dev CA that signs `cert:` leaves (CAROOT) isn't in the host/browser trust store — the driver no longer mints/rotates a leaf into `.secrets/tls/` | `lo trust` (wraps `mkcert -install`; installs the CAROOT CA, one-time, idempotent) | FIXED (`libs/trust`; cert path is the `cert:` generator `kustomize/.../generator/cert.go`) |
| 3 | literal `${LOK8S_USER_API_HOST}/32` → invalid CIDR | a build path skipped `envsubst` (or a stale artifact) | rebuild with `lo build` (substitutes via the kubeconfig API host); `lo build --split` now runs the same envsubst (`build::targets_split`) | FIXED incl. `--split` (`libs/build`) |
...
| 7 | `docker push … connection refused` during `tilt ci` (pulls work) — only when `spec.registries.tls: false` | host `daemon.json` `insecure-registries` CIDR doesn't cover the lok8s registry IPs | widen the CIDR (e.g. `10.125.0.0/16`) + restart dockerd | HOST (lok8s' own cloud-init template is correct) |
